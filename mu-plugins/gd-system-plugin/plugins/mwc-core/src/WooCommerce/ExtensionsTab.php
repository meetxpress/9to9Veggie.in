...
class ExtensionsTab
{
    ...
    {
        if (! $this->shouldInitialize() || ! ManagedExtensionsRepository::getManagedPlugins()) {
            return;
        }
    ...
    /**
     * Determines whether this class should be initialized or not.
     *
     * @since x.y.z
     *
     * @return bool
    ...
    {
        return class_exists('WC_Helper_Updater') && ManagedWooCommerceRepository::hasEcommercePlan();
    }
    ...
    public function maybeRemoveManagedPluginsFromFeatured($transient)
    {
        ...
        foreach ($transient->sections as $key => &$section) {
            ...
            $section->items = ArrayHelper::where($section->items, function ($item) use ($pluginBasenames) {
                return ! in_array($item->plugin, $pluginBasenames);
            });
