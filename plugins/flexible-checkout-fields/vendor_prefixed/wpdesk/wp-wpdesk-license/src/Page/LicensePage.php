 ...
 */
class LicensePage implements \FcfVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FcfVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('FcfVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FcfVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
