 ...
 * @author Gyula Sallai
 */
class SwiftMailerHandler extends \FcfVendor\Monolog\Handler\MailHandler
{
    protected $mailer;
     ...
     */
    public function __construct(\FcfVendor\Swift_Mailer $mailer, $message, $level = \FcfVendor\Monolog\Logger::ERROR, $bubble = \true)
    {
    ...
    {
        return new \FcfVendor\Monolog\Formatter\LineFormatter($format);
    }
    /**
    ...
    {
        $message = null;
        if ($this->messageTemplate instanceof \FcfVendor\Swift_Message) {
            $message = clone $this->messageTemplate;
            $message->generateId();
        } elseif (\is_callable($this->messageTemplate)) {
            $message = \call_user_func($this->messageTemplate, $content, $records);
        }
        if (!$message instanceof \FcfVendor\Swift_Message) {
