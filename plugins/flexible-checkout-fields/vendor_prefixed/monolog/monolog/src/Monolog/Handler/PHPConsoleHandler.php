 ...
 */
class PHPConsoleHandler extends \FcfVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FcfVendor\PhpConsole\Connector $connector = null, $level = \FcfVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FcfVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FcfVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FcfVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FcfVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FcfVendor\PhpConsole\Helper::isRegistered()) {
            \FcfVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FcfVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FcfVendor\Monolog\Logger::NOTICE) {
    ...
    {
        return new \FcfVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
